...
def calculate_wager_array(er, odd, risk):
    return list(map(lambda x: round(er / x.f_odd, 2), odd))


def calculate_return_array(wager_array, odd):
    output = []
    for i in range(0, len(wager_array)):
        output[i] = round(wager_array[i] * odd[i], 2)
    return output


def analyze_with_odds(min_expected_wager,
                      max_expected_wager,
                      odds,
                      topx=10,
                      risk=[],
    ...
    output = []
    for odd in odds:
        for er in range(min_expected_wager, max_expected_wager, 20):
            wager_array = calculate_wager_array(er, odd, risk)
            profit = round((er - round(sum(wager_array), 2)) / 100, 0)
            if profit > min_profit_percentage:
                # Calculate Return array
                return_array = calculate_return_array(wager_array, odd)
                # Calculate ROI array
                roi_array = calculate_roi_array(return_array, round(sum(wager_array), 2))
                output += [ROI(wager_array, odd, roi_array, return_array)]
                print("Found positive profit: ", wager_array, odd)
            else:
                output += [ROI(wager_array, odd, [], [])]
...
# def zip_wagers_to_odds(odds, total_wager, a, b, risk=[]):
#     output = []
#     # print(*odds, sep="\n\n")
#     for i in range(a, b):
#         if i < len(wagers):
